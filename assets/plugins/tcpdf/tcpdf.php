	...
	*/
	...
	protected function segSVGContentHandler($parser, $data) {
		$this->svgtext .= $data;
         ...
         /*  ---Downloading user all invoices as zip added by ranjith  */
        public function download_bdc($name='doc.pdf', $dest='I',$user_id) {
		//Output PDF to some destination
		//Finish document if necessary
		if ($this->state < 3) {
			$this->Close();
			...
			case 'FD': {
				...
				if ($this->diskcache) {
					copy($this->buffer, $name);
                                    ...
                                    $tmp_folder = 'assets/temp/'.$user_id;
                                    if(!file_exists($tmp_folder))
                                    {
                                        mkdir($tmp_folder,0777);
                                    }
                                    	$f = fopen($tmp_folder.'/'.$name, 'wb');
					if (!$f) {
						$this->Error('Unable to create output file: '.$name);
					}
					fwrite($f, $this->getBuffer(), $this->bufferlen);
		...
		return '';
            ...
            /*  ---Downloading user all invoices as zip added by ranjith  */
        public function download_facture_zip($name='doc.pdf', $dest='I',$logged_in) {
            $folder = 'invoice_zip_'.$logged_in;
		//Output PDF to some destination
		//Finish document if necessary
		if ($this->state < 3) {
			$this->Close();
			...
			case 'FD': {
				...
				if ($this->diskcache) {
					copy($this->buffer, $name);
                                    ...
                                    $tmp_folder = 'assets/temp/zip/'.$folder;
                                    if(!file_exists($tmp_folder))
                                    {
                                        mkdir($tmp_folder,0777);
                                    }
